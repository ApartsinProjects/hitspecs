...
TRACKS = {
    ...
    "QCF": dict(
        name="AI and Quantum Computing for Finance",
        he="בינה מלאכותית וחישוב קוונטי למערכות פיננסיות",
        nav="AI & Quantum",
...
def nav_html(active, p):
  ...
  <div class="wrap">
    ...
    <label for="navtoggle" class="navburger" aria-label="Menu"><span></span><span></span><span></span></label>
    <nav>{items}</nav>
  </div>
</header>"""
...
def build_course(cid):
    ...
    if downloads:
      ...
      <section class="block">
        ...
        <div class="chips download-chips">{links}</div>
      </section>"""
...
.navburger{display:none;flex-direction:column;justify-content:center;gap:5px;cursor:pointer;padding:8px;margin-left:auto}
.navburger span{display:block;width:24px;height:2px;background:#fff;border-radius:2px;transition:transform .2s,opacity .2s}
...
@media(max-width:760px){
  ...
  .topbar nav{display:none;position:absolute;top:100%;left:0;right:0;flex-direction:column;align-items:stretch;
    gap:.12rem;padding:.45rem .7rem .75rem;background:rgba(20,56,92,.99);border-bottom:1px solid rgba(255,255,255,.14);
    box-shadow:0 12px 24px -10px rgba(0,0,0,.4)}
  .navtoggle:checked ~ nav{display:flex}
  .topbar nav a{font-size:.95rem;padding:.6rem .55rem;border-radius:6px}
  .hero{padding:1.7rem 0 1.5rem}
...
.download-chips .chip{display:inline-flex;align-items:center;justify-content:center;min-height:2.25rem;padding:.45rem .75rem;
  border-radius:8px;font-size:.84rem;text-align:center;line-height:1.25}
...
@media(max-width:600px){
  .hero{padding:1.5rem 0 1.4rem}
  .hero h1{font-size:1.85rem}
  .hero .eyebrow{margin-bottom:.4rem}
  .hero .lead{font-size:.98rem}
  .chero{padding:1.8rem 0 1.5rem}
  .chero h1{font-size:1.7rem}
  .brand{font-size:.92rem}
}
@media(max-width:420px){
  .download-chips{display:grid;grid-template-columns:1fr;gap:.45rem}
  .download-chips .chip{width:100%;font-size:.88rem}
}
"""
